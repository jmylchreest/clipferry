...
impl X11 {
    ...
    /// `_NET_SUPPORTING_WM_CHECK`). xwayland-satellite ≤ 0.8 runs its own
    /// partial W→X clipboard sync from this window — claims by it are
    /// another bridge mirroring the Wayland selection, not a real owner.
